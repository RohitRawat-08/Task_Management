...
def home(request):
    tasks_list = TodoModel.objects.filter(user = request.user)
...
def addtask(request):
    ...
    if request.method == "POST":
        ...
        TodoModel.objects.create(title=title,desc=desc,user=request.user)
...
def taskHistory(request):
    ...
    data = DeleteModel.objects.filter(user=request.user)
...
def conform(request,id):
    ...
    if request.method == 'POST':
        DeleteModel.objects.create(title=de.title,desc=de.desc,user=request.user)
        de.delete()
...
def Complete_btn_on_home(request,id):
    ...
    CompleteModel.objects.create(title=com.title,desc=com.desc,user=request.user)
    com.delete()
    return redirect('taskcomplete')
...
def conform_DeleteAll_Task_history(request):
    if request.method == 'POST':
        all_data = DeleteModel.objects.filter(user=request.user)
        all_data.delete()
        return redirect('taskhistory')
...
def restore_single_history_task(request,id):
    ...
    TodoModel.objects.create(title=data.title,desc=data.desc,user=request.user)
    data.delete()
...
def Restore_all_HistoryTask(request):
    data = DeleteModel.objects.all()
    for i in data:
        TodoModel.objects.create(title=i.title,desc=i.desc,user=request.user)
    data.delete()
